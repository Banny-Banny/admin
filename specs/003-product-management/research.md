...
**Question**: 상품 관리 API를 어떻게 구조화할 것인가?

**Decision**: 기존 inquiry API 구조를 따라 `src/app/commons/apis/product/` 폴더에 `http.ts`와 `index.ts`를 생성

**Rationale**:
- 기존 `src/app/commons/apis/inquiry/http.ts` 구조와 일관성 유지
- `src/app/commons/apis/admin/index.ts` 패턴 참고
- 기존 `apiClient` 클래스 재사용 (토큰 인터셉터 포함)
- 타입 안정성을 위한 TypeScript 인터페이스 정의

**Alternatives considered**:
- **별도 API 클라이언트 생성**: 불필요한 중복 코드
- **admin 폴더에 통합**: 상품 관리가 독립적인 도메인이므로 분리하는 것이 적절
...
**Question**: 상품 엔티티의 TypeScript 타입을 어떻게 정의할 것인가?

**Decision**: API 응답 형식에 맞춰 타입 정의, `src/app/commons/types/product.ts`에 생성
...
**Type Structure**:
```typescript
// Product: API 응답의 data 필드 구조
// ProductListResponse: GET /api/admin/products 응답
// ProductDetailResponse: GET /api/admin/products/{id} 응답
// CreateProductRequest: POST /api/admin/products 요청
// UpdateProductRequest: PATCH /api/admin/products/{id} 요청
```
...
**Question**: 검색어와 필터 조건을 어떻게 API에 전달할 것인가?

**Decision**: Query 파라미터로 전달, 기존 ProductList 컴포넌트의 로컬 필터링을 API 필터링으로 변경

**Rationale**:
- API가 이미 `search`, `categoryId`, `status`, `limit`, `offset` 파라미터 지원
...
- `status`: 상태 필터 (ALL, ACTIVE, INACTIVE, DELETED)
- `limit`: 페이지 크기 (기본값: 20)
...
**Question**: 기존 ProductsPage와 ProductList 컴포넌트를 어떻게 수정할 것인가?
...
**Modification Points**:
1. `ProductsPage`: 상품 등록 API 호출 추가
2. `ProductList`: 상품 목록 조회 API 호출 추가, 검색/필터링을 API 파라미터로 전달
3. 상품 수정/삭제 기능 추가 (현재 UI에 없음)
...
**Decision**: 기존 `src/app/tests/api-tests/inquiry-test/` 구조를 따라 `product-test/` 폴더 생성
...
```typescript
// product-api.spec.ts
// - GET /api/admin/products 테스트
// - POST /api/admin/products 테스트
// - GET /api/admin/products/{id} 테스트
// - PATCH /api/admin/products/{id} 테스트
// - DELETE /api/admin/products/{id} 테스트
```
...
- HTTP 400: 잘못된 요청
- HTTP 404: 상품을 찾을 수 없음
- HTTP 500: 서버 오류

## 보안 고려사항

1. **토큰 기반 인증**: 모든 API 요청에 Bearer 토큰 포함
...
4. **캐싱**: 상품 목록 캐싱 고려 (선택적)

## 다음 단계

Phase 1에서 다음을 구현:
1. data-model.md: 상품 엔티티 및 API 요청/응답 타입 정의
